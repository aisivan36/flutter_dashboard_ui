...
import 'package:web_dashboard/models/recent_file.dart';

class RecentFileWidget extends StatelessWidget {
  const RecentFileWidget({Key? key}) : super(key: key);
          ...
          SizedBox(
            ...
            child: DataTable2(
              ...
              rows: List.generate(demoRecentFiles.length,
                  (index) => recentFileDatRow(demoRecentFiles[index])),
            ),
          ),
        ],
      ),
...
DataRow recentFileDatRow(RecentFile fileInfo) {
  return DataRow(
    cells: [
      DataCell(
        Row(
